...
def  passGen(user_number) :
    ...
    for x in range(0, user_number, 1):
        ...
        z = z + y
    input("Initial password: "+ z)
    z = rep_with_upper(z)
    z = swap_letters(z)
    input("Final password: "+ z)
    key = input("Enter a key : ")
    z = search_letter(z,key)
    print(z)
    y = False
    return z


def rep_with_upper(x):
    y = len(x)
    z = random.randint(0,y)
    result = x[:z].lower() + x[z:].capitalize()
    print(result)
    return(result)
def swap_letters(x):
    mid2 = x[2:len(x) - 2]
    x = x[len(x) - 2] + x[len(x) - 1] + mid2 + x[0] + x[1]
    print(x)
    return (x)
def search_letter(x,z):
...
y = True
while(y == True):
    String = int(input('Write a number between 3 and 7'))
    if String <= 7 and String >= 3 :
        passGen(String)
        break
    else:
        print("Invalid value!")
